...
contract AlpacaNFTOptimized is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    address public alpacaItemsContract;
    ...
    event AlpacaEvolved(uint256 tokenId, uint8 newStage);
    ...
    function _checkLevelUp(uint256 tokenId) private {
        ...
        if (alpaca.experience >= requiredExp) {
            ...
            if (alpaca.evolutionStage == 0 && alpaca.level >= 10) {
                alpaca.evolutionStage = 1;
                emit AlpacaEvolved(tokenId, 1);
            } else if (alpaca.evolutionStage == 1 && alpaca.level >= 25) {
                alpaca.evolutionStage = 2;
                emit AlpacaEvolved(tokenId, 2);
            }
        }
    }

    function setAlpacaItemsContract(address _address) public onlyOwner {
        alpacaItemsContract = _address;
    }

    function equipItem(uint256 tokenId, uint256 itemId) public {
        require(ownerOf(tokenId) == msg.sender, "Not the owner");
        require(alpacaItemsContract != address(0), "Items contract not set");
        require(IAlpacaItems(alpacaItemsContract).balanceOf(msg.sender, itemId) > 0, "You do not own this item");

        alpacas[tokenId].equipmentId = itemId;
    }

    function unequipItem(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Not the owner");
        alpacas[tokenId].equipmentId = 0;
    }
    ...
    function recordTrade(uint256 tokenId, int256 pnl, bool isWin) public {
        ...
        alpaca.totalTrades++;
        alpaca.totalPnL += pnl;
        if (isWin) { alpaca.wins++; }
        alpaca.experience += 5;
        _checkLevelUp(tokenId);
        emit TradeRecorded(tokenId, pnl);
    }

    // ... (rest of the view, pure, and other functions) ...
